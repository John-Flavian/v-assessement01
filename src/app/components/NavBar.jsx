...
const NavBar = () => {
  ...
  return (
    <div>
      ...
      <nav className="hidden md:block px-[20px] w-[300px] mb-4 border-solid border-2 overflow-y-auto">
        {/* Header Section */}
        <header className="flex mt-5 mb-5">
          <Image src={logo} alt="logo" width={32} height={32} />
          <h1 className="text-[20px] font-semibold">Untitled UI</h1>
        </header>
            ...
            {navLinks.map((link, index) => {
              return (
                <li key={index} className={navLiStyle}>
                  ...
                  {link.badge && (
                    <span className={badgeStyle}>{link.badge}</span>
                  )}
                </li>
              );
            })}
      ...
      <nav className="md:hidden z-30 fixed top-0 right-0 w-full bg-white p-4 flex justify-between items-center">
        ...
        <div
          className={`${
            !toggle ? "hidden" : "flex"
          } absolute bg-gray-200 top-10 p-5 right-0 mx-4 my-2 min-w-[250px] rounded-xl`}
        >
          <ul className="list-none flex w-full flex-col gap-1">
            {navLinks.map((link, index) => {
              return (
                <li key={index} className={navLiStyle}>
                  {link.icon}
                  <p className="text-gray-900">{link.title}</p>
                  {link.badge && (
                    <span className={badgeStyle}>{link.badge}</span>
                  )}
                </li>
