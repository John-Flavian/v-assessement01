const iconStyle = "mr-2 text-gray-500 text-[30px]";
const navLiStyle =
  "relative flex items-center transition-bg duration-300 hover:bg-gray-100 hover:font-bold py-2 rounded-lg";
const badgeStyle =
  "absolute ml-2 bg-gray-300 text-black text-xs px-3 py-1 rounded-full top-3.5 right-5 transform translate-x-1/2 -translate-y-1/2";

const dashboardNavUlStyle = "list-none flex border border-gray-300 rounded-lg";
const dashboardNavLiStyle =
  "font-medium transition-bg duration-300 hover:bg-gray-100 hover:font-semibold text-gray-700 cursor-pointer border-gray-200 p-3 text-center";
const paymentDivStyle =
  "relative flex items-start py-5 px-2 w-full border-solid border-2 shadow-md rounded-lg";

const headerCell1 = "table-cell w-1/8 border-b py-2 text-xs text-gray-500";
const headerCell2 = "table-cell w-2/8 border-b py-2 text-xs text-gray-500";

const contentCell1 = "table-cell w-1/8 border-b py-6 text-sm text-gray-500";
const contentCell2 = "table-cell w-2/8 border-b py-6 text-sm";

export {
  iconStyle,
  navLiStyle,
  badgeStyle,
  dashboardNavLiStyle,
  dashboardNavUlStyle,
  paymentDivStyle,
  headerCell1,
  headerCell2,
  contentCell1,
  contentCell2,
};
